...
public class Colorlist extends JPanel {
    ...
    public void addColorlist(){
        // 得到监听器实例
        Toolbar.EventListener el = Toolbar.getInstance().el;
        //System.out.println("ColorList linked to " + el.num);
        // 为列表使用二行四列的栅格布局
        this.setLayout(new GridLayout(2, 4, 2, 2));
        // 通过颜色数组快速构建前七个按钮
        Color[] colorArray = { Color.BLACK, Color.BLUE, Color.YELLOW, Color.GREEN, Color.WHITE, Color.RED, Color.CYAN };
        ...
        customColor.setBackground(Color.black);
        // 为该按钮加入与其它按钮相同的监听
...
//    public Colorlist() {
//        // 得到监听器实例
//        System.out.println("ColorList linked to " + Window.now);
//        // 为列表使用二行四列的栅格布局
//        this.setLayout(new GridLayout(2, 4, 2, 2));
//        // 通过颜色数组快速构建前七个按钮
